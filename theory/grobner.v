...
Definition plt p q : bool := (sort >=%O (msupp p) < sort >=%O (msupp q))%O.
...
suff F : 
  reflect
    (exists m,
       [/\ m \in sort >=%O (msupp q), m \notin sort >=%O (msupp p)
        & forall m1, (m < m1)%O -> 
            (m1 \in sort >=%O (msupp p) = (m1 \in sort >=%O (msupp q)))])
    (sort >=%O (msupp p) < sort >=%O (msupp q))%O.
  pose Fx := perm_mem (permEl (perm_sort _ _)).
  apply: (iffP F) => [] [m [H1m H2m H3m]]; exists m; split => //.
  - by rewrite Fx in H1m.
  - by rewrite Fx in H2m.
  - by move=> m1 /H3m; rewrite !Fx.
  - by rewrite Fx.
  - by rewrite Fx.
  by move=> m1 /H3m; rewrite !Fx.
have : sorted >%O (sort >=%O (msupp p)).
  rewrite -[sort _ _]revK rev_sorted /= lt_sorted_uniq_le.
  rewrite rev_uniq sort_uniq msupp_uniq /=.
  by rewrite rev_sorted /= sort_sorted //=; apply: ge_total.
have : sorted >%O (sort >=%O (msupp q)).
  rewrite -[sort _ _]revK rev_sorted /= lt_sorted_uniq_le.
  rewrite rev_uniq sort_uniq msupp_uniq /=.
  by rewrite rev_sorted /= sort_sorted //=; apply: ge_total.
elim: (sort _ (msupp q)) (sort _ (msupp p)) => /=.
  move=> s _ _; apply: (iffP idP); first by rewrite ltx0.
  by case => m [] /=; rewrite in_nil.
move=> a l1 IH [|b l2] /= Pl1 Pl2.
  apply: (iffP idP) => //.
  move=> _; exists a; split=> //; first by rewrite in_cons eqxx.
  move=> m1 aLm1; rewrite in_nil in_cons.
  case: eqP; intros He; first by rewrite He ltxx in aLm1.
  case: (boolP (_ \in _)) => //= m1Il1.
  have/allP F : all (< a)%O l1.
    apply: order_path_min _ Pl1.
    by apply: rev_trans (lt_trans).
  by rewrite ltNge le_eqVlt negb_or [(_ < _)%O]F // andbF in aLm1.
have/allP F : all (< a)%O l1.
  apply: order_path_min _ Pl1.
  by apply: rev_trans (lt_trans).
have/allP G : all (< b)%O l2.
  apply: order_path_min _ Pl2.
  by apply: rev_trans (lt_trans).
have [/eqP aEb|aDb] := boolP (a == b).
  rewrite -aEb in Pl2 G *.
  have Hl1 : sorted >%O l1 by apply: path_sorted Pl1.
  have Hl2 : sorted >%O l2 by apply: path_sorted Pl2.
  have {}IH := IH _ Hl1 Hl2.
  rewrite eqhead_ltxiE.
  apply: (iffP IH) => [] [x [H1x H2x H3x]]; exists x.
    split; first  by rewrite in_cons H1x orbT.
      by rewrite in_cons negb_or H2x neq_lt [(_ < a)%O]F.
    move=> m1 xLm1; rewrite !in_cons.
    by case: eqP => //= _; apply: H3x.
  rewrite in_cons negb_or in H2x.
  have /andP[xDa {}H2x] := H2x.
  split=> //; first by rewrite in_cons (negPf xDa) in H1x.
  move=> m1 xLm1.
  have := H3x _ xLm1.
  rewrite !in_cons; case: eqP => //= m1Ea.
  have /negPf-> : m1 \notin l1 by apply/negP=> /F; rewrite m1Ea /= ltxx.
  by have /negPf-> : m1 \notin l2 by apply/negP=> /G; rewrite m1Ea /= ltxx.
rewrite neqhead_ltxiE; last by rewrite eq_sym.
apply: (iffP idP) => [bLa|].
  exists a; split; rewrite ?in_cons; first by rewrite eqxx.
    rewrite negb_or aDb /=.
    apply/negP => /G /=.
    by rewrite ltNge le_eqVlt negb_or bLa andbF.
  move=> m1 aLm1; rewrite !in_cons.
  case: eqP => /= [m1Eb|_].
    by rewrite m1Eb ltNge le_eqVlt negb_or bLa andbF in aLm1.
  case: eqP => /= [m1Ea|_].
    by rewrite m1Ea ltxx in aLm1.
  apply/idP/idP => [/G/= m1Lb|/F/=].
    have : (a < b)%O by apply: lt_trans m1Lb.
    by rewrite ltNge le_eqVlt negb_or bLa eq_sym aDb.
  by rewrite ltNge le_eqVlt negb_or aLm1 andbF.
case=> m [H1m H2m H3m].
rewrite in_cons in H1m.
have /orP[/eqP mEa|mIl1] := H1m.
  rewrite inE mEa negb_or aDb /= in H2m.
  case: (ltP a b) => [aLb|]; last first.
    by rewrite le_eqVlt eq_sym (negPf aDb).
  suff: b \in l1 by move=> /F.
  rewrite -mEa in aLb.
  have := H3m _ aLb.
  by rewrite !inE eqxx /= eq_sym (negPf aDb).
suff: b \in l1 by move=> /F.
have /H3m := F _ mIl1.
  rewrite !inE eqxx (negPf aDb) /= => /idP aIl2.
have /= aLb := G _ aIl2.
have /= mLa := F _ mIl1.
have := H3m _ (lt_trans mLa aLb).
by rewrite !inE eqxx /= eq_sym (negPf aDb) /=.
Qed.

Lemma mlead_head p : mlead p = head 0%MM (sort >=%O (msupp p)).
Proof.
rewrite /mlead.
have F : perm_eq (msupp p) (sort >=%O (msupp p)).
  by rewrite perm_sym perm_sort.
rewrite (perm_big _ F) /=.
have : sorted >=%O ((sort >=%O (msupp p))).
  by apply/sort_sorted/ge_total.
case: sort => [|a l] /= Hp; first by rewrite big_nil.
rewrite big_cons (join_idPl _) //.
elim: l a Hp => [a _ |b l IH a /= /andP[? ?]].
  by rewrite big_nil // le0x.
by rewrite big_cons (join_idPl _) // IH.
Qed.

Lemma plt_mlead p q : (mlead p < mlead q)%O -> (p < q).
Proof.
rewrite /plt !mlead_head; do 2 case: sort => //=; first by rewrite ltxx.
  by move=> a l; rewrite ltx0.
move=> b l1 a l2 /= aLb.
by rewrite neqhead_ltxiE // neq_lt aLb.
Qed.

Lemma plt_anti p : p < p = false.
Proof. by rewrite [_ < _]ltxx. Qed.

Lemma plt0 p : (0 < p) = (p != 0).
Proof.
rewrite /plt msupp0 {1}/sort /= lt_neqAle andbT -msupp_eq0 /=.
by case: sort (size_sort >=%O (msupp p)); case: msupp.
Qed.

Lemma plt0r p : p < 0 = false.
Proof.
rewrite /plt msupp0 {2}/sort /=.
by case: sort (size_sort >=%O (msupp p)); case: msupp.
Qed.

Lemma plt_trans : transitive plt.
Proof. by move=> p q r H1 H2; exact: (lt_trans H1 H2). Qed. 
