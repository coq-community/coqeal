...
let coqlib_prefix = "Coq."
let ssreflect_prefix = "Ssreflect."
let coqeal_prefix = "CoqEAL."

let coqlib_url = "http://coq.inria.fr/library/"
let ssreflect_url = "http://ssr2.msr-inria.inria.fr/~jenkins/current/"
let coqeal_url = "http://www.maximedenes.fr/CoqEAL/"

let re_coqlib = Str.regexp (Str.quote coqlib_prefix)
let re_ssreflect = Str.regexp (Str.quote ssreflect_prefix)
let re_coqeal = Str.regexp (Str.quote coqeal_prefix)
let re_library () = Str.regexp (Str.quote !library_name ^ ".")

let re_sane_path = Str.regexp "[A-Za-z0-9_.]+$"

let relativize m =
  if Str.string_match (re_library ()) m 0 then
    Str.string_after m (String.length !library_name + 1)
  else m
  ...
  | Ref(m', p, ty) ->
      let url =
        if Str.string_match re_coqlib m' 0 then
          coqlib_url ^ m' ^ ".html"
        else if Str.string_match re_ssreflect m' 0 then
          let m' = Str.string_after m' (String.length ssreflect_prefix) in
          ssreflect_url ^ m' ^ ".html"
        else if Str.string_match re_coqeal m' 0 then
          let m' = Str.string_after m' (String.length coqeal_prefix) in
          coqeal_url ^ m' ^ ".html"
        else if Hashtbl.mem xref_modules m' then
          m' ^ ".html"
        else
          raise Not_found in
      if p = "" then
...
let coq_terms = mkset [
  ...
  "where"; "struct"; "wf"; "measure"; "of"
]

let coq_keywords = mkset [
  "AddPath"; "Axiom"; "Abort"; "Boxed"; "Canonical"; "Chapter"; "Check";
...
let coq_tactics = mkset [
  ...
  "generalize_eqs_vars"; "induction"; "rename"; "move";
  "set"; "assert"; "do"; "repeat"; "cut"; "have"; "suff"; "wlog";
  "split"; "subst"; "try"; "simpl"; "unfold"; "red"; "congr"; "unlock";
  "compute"; "at"; "in"; "symmetry"; "exists"; "pose"; "constructor";
  "transitivity"; "replace"; "setoid_replace"; "inversion";
  "inversion_clear"; "pattern"; "intuition"; "congruence"; "fail";
  "fresh"; "trivial"; "firstorder"; "ring";
  "clapply"; "program_simpl"; "program_simplify"; "eapply"; "auto";
  "eauto"; "Time"; "vm_compute"; "native_compute"; "hnf"
]

let coq_terminators = mkset [
   "exact"; "by"; "done"; "reflexivity"; "tauto"; "romega"; "omega";
...
let ident pos id =
  if StringSet.mem id coq_terms then
    fprintf !oc "<span class=\"term\">%s</span>" id
  else if StringSet.mem id coq_keywords then
    fprintf !oc "<span class=\"kwd\">%s</span>" id
  else if StringSet.mem id coq_tactics then
    fprintf !oc "<span class=\"tactic\">%s</span>" id
  else if StringSet.mem id coq_terminators then
    fprintf !oc "<span class=\"terminator\">%s</span>" id
...
let process_file f =
  if Filename.check_suffix f ".v" then begin
    (* If the library was compiled with a -R argument,
       the initial current_module is wrong and all crossrefs fail *)
    current_module := sprintf "%s.%s" !library_name
      (Filename.chop_suffix (Filename.basename f) ".v");
    let modname = relativize !current_module in
    let ic = open_in f in
    if !output_name = "-" then
      oc := stdout
    else
      oc := open_out (Str.global_replace (Str.regexp "%") modname
                                                          !output_name);
    start_html_page modname;
...
let _ =
  Arg.parse [
    ...
    "-R", Arg.String (fun s -> library_name := s),
      " <library>  Set library name"
  ] process_file
