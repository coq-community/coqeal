...
Variable (rpos : refinement Rpos).
Ltac rpos := have ? := rpos.

Lemma RposE (p : pos) (x : positive) : param Rpos p x -> p = pos_of_positive x. 
Proof. by rpos; rewrite paramE; case. Qed.
...
Proof. by rpos; rewrite !paramE; apply: val_inj; rewrite /= insubdK. Qed.
...
Proof.
rpos; rewrite paramE => _ x <- _ y <-; apply: val_inj.
...
Proof.
rpos; rewrite paramE => _ x <- _ y <-; apply: val_inj; rewrite !val_insubd.
...
Proof.
rpos; rewrite paramE => /= _ x <- _ y <-; rewrite /leq_pos !val_insubd.
move: (Pos2Nat.is_pos x) (Pos2Nat.is_pos y) => /leP -> /leP ->.
...
Proof.
rpos; rewrite paramE => /= _ x <- _ y <-; rewrite /lt_pos !val_insubd.
move: (Pos2Nat.is_pos x) (Pos2Nat.is_pos y) => /leP -> /leP ->.
...
Proof.
rnat; rewrite paramE => _ x <- _ y <-; rewrite /Rnat /fun_hrel /=.
...
Proof.
rnat; rewrite paramE => _ x <- _ y <-; rewrite  /eq_op /eq_N.
...
Proof.
rnat; rewrite paramE => x x' rx y y' ry; rewrite /leq_op /leq_N.
...
Proof.
rnat; apply param_abstr2 => x x' rx y y' ry; rewrite paramE /Rnat /fun_hrel.
...
Proof.
rnat; apply param_abstr => x x' rx; rewrite paramE /cast /Rnat /fun_hrel.
...
Proof. 
rnat; apply param_abstr => x x' rx; rewrite paramE [x]RnatE.
