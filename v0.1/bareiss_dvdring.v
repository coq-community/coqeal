...
Require Import dvdring minor atomic_operations.
...
move => a b hab z h h'.
apply/hab.
by rewrite -mulrA h' mulrA.
Qed.

Lemma monic_lreg : forall p: {poly R}, p \is monic -> lreg p.
Proof.
rewrite /monic => p /eqP h.
apply lreg_lead; rewrite h.
by apply lreg1.
Qed.
...
Lemma Bareiss_recE : forall m a (M: 'M[R]_(1 + m)),
 ...
 (forall p  (h h' :p.+1 <= 1 + m),
   lreg (minor (step_fun h) (step_fun h') M)) ->
...
have : forall p (h h': p.+1 <= 1 + (1 + m)),
  lreg (minor (step_fun h) (step_fun h') M).
- rewrite -heq => p h h'.
  rewrite (@minor_eq _ _ _ _ _ (step_fun h) _ (step_fun h)) ?hm//.
...
rewrite (@minor_eq _ _ _ _ _ (step_fun h) _ (step_fun h)); first last.
