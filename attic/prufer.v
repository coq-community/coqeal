...
From mathcomp Require Import path ssralg fintype perm choice.
From mathcomp Require Import matrix bigop zmodp mxalgebra poly.

Require Import ssrcomplements dvdring stronglydiscrete coherent.
...
Definition pmember n (x : R) : 'cV[R]_n -> option 'rV[R]_n := match n with
  | S p => fun (I : 'cV[R]_p.+1) =>
      let a := plm (I ^T) in
      if [forall i, I i 0 %| a i i * x]
         then Some (\row_i odflt 0 (a i i * x %/? I i 0))
         else None
  | _ => fun _ => if x == 0 then Some 0 else None
end.

Lemma pmember_correct : forall n (x : R) (I : 'cV[R]_n),
...
case=> [|n] x I; rewrite /pmember.
  ...
  by move => J; rewrite mulmx0 -scalemx1 scalemx_eq0 h oner_neq0.
set M := plm _.
case: ifP => /forallP hdvd; constructor.
  apply/rowP => i; rewrite !ord1 !mxE mulr1n  {i}.
  case: (plmP (I^T)); rewrite /isPLM /P1.
...
rewrite big_distrr /=.
suff -> : \sum_j (M i i * (J 0 j * I j 0)) = \sum_j (I i 0 * (J 0 j * M i j))
  by rewrite -big_distrr dvdr_mulr.
apply/eq_big => // j _.
case: (plmP (I ^T)); rewrite /isPLM /P2 => _ /(_ j i i).
by rewrite !mxE mulrCA => ->; rewrite !mulrA mulrC mulrA.
Qed.
...
Definition inv_id n (i : 'I_n) (I : 'cV[R]_n) : 'cV[R]_n := (row i (plm (I^T)))^T.

Lemma inv_idP1 n (I: 'cV[R]_n) j : (inv_id j I *i I <= (I j 0)%:M)%IS.
Proof.
