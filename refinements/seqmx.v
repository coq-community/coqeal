...
Definition hseqmx := fun (_ _ : nat) => seqmx.
...
CoInductive Rseqmx {m1 m2} (rm : nat_R m1 m2) {n1 n2} (rn : nat_R n1 n2) :
  'M[R]_(m1,n1) -> hseqmx R m2 n2 -> Type :=
...
Instance Rseqmx_map_seqmx m1 m2 (rm : nat_R m1 m2) n1 n2 (rn : nat_R n1 n2) :
  refines (eq ==> Rseqmx rm rn ==> Rseqmx rm rn)
     (fun (f : R -> R) => @matrix.map_mx R R f m1 n1) map_mx_op.
Proof.
...
Proof.
  rewrite refinesE=> _ _ [M sM h1 h2 h3].
  rewrite /spec /spec_seqmx /spec_id /spec /specR /spec_id map_id_in;
    last first.
...
Definition RseqmxC {m1 m2} (rm : nat_R m1 m2) {n1 n2} (rn : nat_R n1 n2) :
  'M[R]_(m1, n1) -> hseqmx C m2 n2 -> Type :=
...
Global Instance RseqmxC_map_mx m1 m2 (rm : nat_R m1 m2) n1 n2 (rn : nat_R n1 n2)
  : refines ((rAC ==> rAC) ==> RseqmxC rm rn ==> RseqmxC rm rn)
          (fun f => @matrix.map_mx _ _ f m1 n1) map_mx_op.
Proof. param_comp map_seqmx_R. Qed.

Global Instance refine_map_seqmx m n :
  refines ((rAC ==> rAC) ==> RseqmxC (nat_Rxx m) (nat_Rxx n) ==>
                         RseqmxC (nat_Rxx m) (nat_Rxx n))
          (fun f => @matrix.map_mx _ _ f m n) map_mx_op.
